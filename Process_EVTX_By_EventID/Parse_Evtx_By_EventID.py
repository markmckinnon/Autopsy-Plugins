# This python autopsy module will export all the Windows Event Logs and then call
...
# view section of Autopsy as a Table based on Event_Log_Id.  The user can then run
# the module again and extract user supplied events from the Evtx SQLite database.
#
...
# Event Log module to parse the Windows Event Logs based on Event Ids.
# July 2017
# 
# Comments 
...
class ParseEvtxByEventIDIngestModuleFactory(IngestModuleFactoryAdapter):

    def __init__(self):
        self.settings = None

    moduleName = "ParseEvtxByEventID"
    
    def getModuleDisplayName(self):
        return self.moduleName
    
    def getModuleDescription(self):
        return "Parses EVTX files By EventID"
    ...
    def createDataSourceIngestModule(self, ingestOptions):
        return ParseEvtxByEventIDIngestModule(self.settings)

# Data Source-level ingest module.  One gets created per data source.
class ParseEvtxByEventIDIngestModule(DataSourceIngestModule):

    _logger = Logger.getLogger(ParseEvtxByEventIDIngestModuleFactory.moduleName)
