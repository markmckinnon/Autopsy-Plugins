# This python autopsy module will export all the thumbcache_*.db files im the image and
# then run the thumbcache_viewer_cmd program, written by Eric Kutcher (https://github.com/thumbcacheviewer/thumbcacheviewer), 
...
class ThumbcacheIngestModuleFactory(IngestModuleFactoryAdapter):

    def __init__(self):
        self.settings = None

    moduleName = "Thumbscache Parser Module"
    ...
    def createDataSourceIngestModule(self, ingestOptions):
        return ThumbcacheIngestModule(self.settings)

# Data Source-level ingest module.  One gets created per data source.
class ThumbcacheIngestModule(DataSourceIngestModule):

    _logger = Logger.getLogger(ThumbcacheIngestModuleFactory.moduleName)
    ...
    def startUp(self, context):
        ...
        self.path_to_exe_thumbs = os.path.join(os.path.dirname(os.path.abspath(__file__)), "thumbcache_viewer_cmd.exe")
    ...
    def check_derived_existance(self, dataSource, file_name, parent_file_abstract):
	    ...
	    #self.log(Level.INFO, "File Exists with File Path ==> " + str(parent_file_abstract));
            return True
	    '''for file in derived_file:
	        if parent_file_abstract.parentPath == file.parentPath
